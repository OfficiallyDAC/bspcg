...
This error might be due to the fact that at this moment (May 2023), there are no official binary releases for Windows and `jaxlib` must be built from source.
...
* `1.1_performances_synth_experiments.pkl` contains the performances depicted in Fig.1 (Section 5) of the paper;
...
* `2.0_results_bef.pkl`, `2.0_results_aft.pkl`, `2.0_Rl2_ours_bef.pkl`, and `2.0_Rl2_ours_aft.pkl` contain the results underlying Fig.2 (Section 6) in the paper.
...
In addition, the results obtained from the mere application of the _Naive_ estimator shown in Supplementary Section 7, can be reproduced from this file as well.